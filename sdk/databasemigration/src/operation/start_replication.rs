...
builder = _header_serialization_settings.set_default_header(
                    ...
                    "AmazonDMSv20160101.StartReplication"
                );
builder
                    };
...
impl StartReplicationError {
    ...
    /// Returns `true` if the error kind is `StartReplicationError::AccessDeniedFault`.
    pub fn is_access_denied_fault(&self) -> bool {
        matches!(self, Self::AccessDeniedFault(_))
    }
    /// Returns `true` if the error kind is `StartReplicationError::InvalidResourceStateFault`.
    pub fn is_invalid_resource_state_fault(&self) -> bool {
        matches!(self, Self::InvalidResourceStateFault(_))
    }
    /// Returns `true` if the error kind is `StartReplicationError::ResourceNotFoundFault`.
