...
pub fn ser_recurrence_settings(
    ...
    if let Some(var_9) = &input.daily_settings {
     ...
     {
        object.key("NumberOfOnCalls").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((input.number_of_on_calls).into()));
    }
    if let Some(var_13) = &input.shift_coverages {
     ...
     {
        object.key("RecurrenceMultiplier").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((input.recurrence_multiplier).into()));
    }
    Ok(())
}
...
pub(crate) fn de_recurrence_settings<'a, I>(tokens: &mut ::std::iter::Peekable<I>) -> Result<Option<crate::types::RecurrenceSettings>, ::aws_smithy_json::deserialize::error::DeserializeError>
    ...
    match tokens.next().transpose()? {
        Some(::aws_smithy_json::deserialize::Token::ValueNull { .. }) => Ok(None),
            ...
            Ok(Some(crate::serde_util::recurrence_settings_correct_errors(builder).build().map_err(|err|::aws_smithy_json::deserialize::error::DeserializeError::custom_source("Response was invalid", err))?))
        }
        _ => {
