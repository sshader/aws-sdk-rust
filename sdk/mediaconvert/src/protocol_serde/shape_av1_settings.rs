...
pub fn ser_av1_settings(
    ...
    if let Some(var_8) = &input.gop_size {
        object.key("gopSize").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::Float((*var_8).into()));
    }
    if let Some(var_9) = &input.max_bitrate {
        object.key("maxBitrate").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_9).into()));
    }
    if let Some(var_10) = &input.number_b_frames_between_reference_frames {
        object.key("numberBFramesBetweenReferenceFrames").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_10).into()));
    }
    ...
    if let Some(var_14) = &input.slices {
        object.key("slices").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_14).into()));
    }
                        ...
                        match key.to_unescaped()?.as_ref() {
                            ...
                            "filmGrainSynthesis" => {
                                builder = builder.set_film_grain_synthesis(
                                    ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?.map(|s|
                                        s.to_unescaped().map(|u|
                                            crate::types::Av1FilmGrainSynthesis::from(u.as_ref())
                                        )
                                    ).transpose()?
                                );
                            }
